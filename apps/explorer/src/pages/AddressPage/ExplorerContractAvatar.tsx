...
import { AlphscanAssetLogo, isLpContractRow, isStrictFungibleTokenIssuer } from '@alphscan/sdk-react-ui'
...
const ExplorerContractAvatar = ({
  ...
  if (isStrictFungibleTokenIssuer(fullRow) && row.token_logo_uri?.trim()) {
    return (
      <AlphscanAssetLogo variant="token" imageUrl={row.token_logo_uri} fallbackChar={fb.slice(0, 1)} size={size} />
    )
  }
  ...
  return <AlphscanAssetLogo variant="token" imageUrl={null} fallbackChar={fb.slice(0, 1)} size={size} />
}
